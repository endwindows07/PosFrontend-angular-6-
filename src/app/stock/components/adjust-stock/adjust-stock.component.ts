...
export class AdjustStockComponent {
  ...
  onAdjustAmountProduct() {
    this.productService.onUpdateProduct(this.form.value, this.productId, this.accessTokenService.getAccesstokenStore())
                       .then(res => {
                         this.alert.success_alert("ปรับยอดสินเค้าสำเร็จ");
                         this.router.navigate(['/', AppUrl.Stock, ProductUrl.DetailProduct, this.productId]);
                       })
                       .catch(err => {
  ...
  initailLoadProduct() {
    this.productService.onGetProductById(this.productId, this.accessTokenService.getAccesstokenStore())
